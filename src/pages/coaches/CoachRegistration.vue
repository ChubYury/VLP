    ...
    methods: {
      validatorInp(keyValue) {
        if (keyValue === 'firstNameVal') {
          ...
          this.isValid.rateValid = this.rateVal === '' ? false : true;
      ...
      setNewCoach() {
        ...
        if (validForm === undefined) {
          const idCoach = `c${this.$store.getters['coaches/setCoaches'].length + 1}`
          const newCoach = {
            id: idCoach,
          ...
          this.$store.dispatch('coaches/getNewCoach', newCoach)
        }
